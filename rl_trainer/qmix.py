...
class QMIX(object):
    def __init__(self, args):
        ...
        self.eval_Q_net=self.eval_Q_net.cuda();
        self.target_Q_net=self.target_Q_net.cuda();
        self.eval_mix_net=self.eval_mix_net.cuda();
        self.target_mix_net=self.target_mix_net.cuda();
        ...
        with torch.no_grad():
            if np.random.uniform() < epsilon:  # epsilon-greedy
                ...
                if self.add_last_action:
                    ...
                    #print(np.shape(last_a_n));
                    inputs.append(last_a_n)
                if self.add_agent_id:
                    inputs.append(torch.eye(6))
                
                '''for x in inputs:
                    print('x',x);'''

                inputs = torch.cat([x for x in inputs], dim=-1).cuda()  # inputs.shape=(N,inputs_dim)
